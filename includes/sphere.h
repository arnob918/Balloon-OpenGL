...
public:
    ...
    float shininess;
    // ctor/dtor
    Sphere(float radius = 0.5f, int sectorCount = 18, int stackCount = 18, glm::vec3 amb = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 diff = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f) : verticesStride(24)
    {
...
private:
    ...
    void buildCoordinatesAndIndices()
    {
        ...
        {
            stackAngle -= stackStep;        // starting from pi/2 to -pi/2ds
            if (i <= stackCount / 2) {
                ...
                y = -(2 * radius / stackCount) * (i - stackCount / 2);
                xz = radius + y;
                lengthInv = 1.0f / sqrtf(y*y + xz*xz);
            }
